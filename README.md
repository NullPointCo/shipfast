...
python3 shipfast.py create my-webapp --stack nextjs-paddle
python3 shipfast.py create my-remix --stack remix-stripe
```
...
| `fastapi-lemonsqueezy` | FastAPI + PostgreSQL + Lemon Squeezy | Merchant-of-record payments (no Stripe/business needed), JWT auth, SQLAlchemy models, checkout API, HMAC-verified webhook, Docker, Render, tests |
...
| `nextjs-paddle` | Next.js 14 + Paddle Billing | App Router, TypeScript, Paddle Checkout (subscriptions), signed webhooks, Docker, pricing page |
| `remix-stripe` | Remix 2 + Stripe | Loaders/actions auth, Stripe Checkout (subscriptions), signed webhooks, TypeScript, Docker |

All 15 stacks ship with auth, a database layer, payment integration (Stripe /
Lemon Squeezy / Paddle), Docker for local dev, a deploy blueprint (Render), and
...
## Roadmap
- More payment providers (Polar, Chargebee)
- Edge runtime templates (Cloudflare Workers + Stripe)
